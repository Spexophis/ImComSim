...
def run_pipeline(image_paths, edge_roi=None):
    ...
    ax0.set_xlabel("Col (px)");
    ax0.set_ylabel("Row (px)")
    ...
    ax1.set_xlabel("u (cy/px)");
    ax1.set_ylabel("v (cy/px)")
    ...
    if ptc_data is not None:
        ...
        ax2.set_xlabel("Mean signal (DN)");
        ax2.set_ylabel("Variance (DN²)")
    ...
    ax4.set_xlabel("f (cy/px)");
    ax4.set_ylabel("NPS (DN²)")
    ax4.set_xlim(0, 0.5);
    ...
    ax6.set_xlabel("f (cy/px)");
    ax6.set_ylabel("DQE")
    ax6.set_xlim(0, 0.5);
    ax6.set_ylim(-0.02, max(1.05, dqe.max() * 1.1))
